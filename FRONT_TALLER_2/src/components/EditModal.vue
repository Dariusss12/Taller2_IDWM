          ...
          <div class="flex min-h-full items-center justify-center p-4">
            ...
            >
              <DialogPanel class=" w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white p-6 shadow-xl transition-all" :key="componentKey">
                <h1 class="text-center mb-10 text-2xl font-bold">Editar a {{ props.name }} {{ props.lastname }}</h1>       
                <form @submit.prevent="submitForm" class="mx-auto w-full">
                  ...
                  <div class="mb-5 ">
                    ...
                    <input type="text" id="price" v-model="formData.lastname" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 " :placeholder=props.lastname>
                    <p v-if="errors.lastname" class="text-red-500 text-sm my-2">{{ errors.lastname[0] }}</p>
                  </div>
                  <div class="mb-5 ">
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-900">Correo Electrónico</label>
                    <input type="text" id="price" v-model="formData.email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 " :placeholder=props.email novalidate>
...
import type { FormEditUser, FormErrors } from '@/interface/interfaces';
import { editUser } from '@/backend/crud';
...
async function submitForm(): Promise<void> {
  ...
  try {
    if(props.id != null){
      await editUser(formData.value, props.id);
      closeModal();
